...
left outer join dd03vv as X on X.tabname = concat(replace(MD.infocube,'/CPMB/','/B28/A'),'7') and 
                               X.fieldname = concat('/B28/S_',substring(D.tech_name,7,12))                                                         
     {
    key D.mandt,                                                                                                
    key D.appset_id,                                                                                     
    key DA.application_id,
    key D.dimension,
    key X.position as SEQNR,
    D.tech_name,
...
// this property is custom and exists only in OBS  
    ...
    concat(concat(concat(D.dimension,' : '),concat('/B28/OI',substring(D.tech_name,7,12))),';') as ztf,
    concat(concat('"',concat(concat(X.fieldname,'" AS ? '),D.dimension)),',') as amdp_select
                           
}                                                                                                    
where D.appset_id = 'TRACTEBEL_TEMIS' and D.mandt = $session.client
